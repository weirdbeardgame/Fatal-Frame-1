// STATUS: NOT STARTED

#ifndef M_NAGAURA_REIKOKU_SRC_GRAPHICS_MOTION_MIM_DAT_H
#define M_NAGAURA_REIKOKU_SRC_GRAPHICS_MOTION_MIM_DAT_H

typedef struct {
	u_int *addr;
	u_char *dat;
} MIME_LOAD;

extern u_char mimchodo_r003[0];
extern u_char mimchodo_r007[0];
extern u_char mimchodo_r009[0];
extern u_char mimchodo_r011[0];
extern u_char mimchodo_r021[0];
extern u_char mimchodo_r041[0];
extern MIME_LOAD mimchodo_tbl[0];


#endif // M_NAGAURA_REIKOKU_SRC_GRAPHICS_MOTION_MIM_DAT_H
