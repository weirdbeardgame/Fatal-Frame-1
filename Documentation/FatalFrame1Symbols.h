...
struct _atexit {
  _atexit *_next;
  int _ind;
  void (*_fns[1])(/* parameters unknown */);
};
...
struct _reent {
  ...
  union {
    ...
    struct {
      unsigned char *_nextf[1];
      unsigned int _nmalloc[1];
    } _unused;
  } _new;
  _atexit *_atexit;
  _atexit _atexit0;
...
struct sce_stat {
  ...
  unsigned char st_ctime[1];
  unsigned char st_atime[1];
  unsigned char st_mtime[1];
  unsigned int st_hisize;
  unsigned int st_private[1];
};
...
typedef int qword[1];
typedef int sceVu0IVECTOR[1];
typedef float sceVu0FVECTOR[1];
typedef float sceVu0FMATRIX[1][1];
...
typedef long int dword[1];
typedef float LMATRIX[1][1];
...
typedef struct {
  ...
  SUBJECT_WRK furn[1];
  SUBJECT_WRK rare[1];
  u_short spcl_pht_score[1];
  byte spcl_pht_no[1][1];
  u_short bonus_sta;
  u_short room;
  u_short dist_2d;
  u_short no_2d;
...
typedef struct {
  ...
  TIM2_UINT32 MMImageSize[1];
} TIM2_MIPMAPHEADER;

typedef struct {
  TIM2_UCHAR8 ExHeaderId[1];
...
typedef int _iconVu0IVECTOR[1];
typedef float _iconVu0FVECTOR[1];
...
typedef struct {
  ...
  _iconVu0IVECTOR BgColor[1];
  _iconVu0FVECTOR LightDir[1];
  _iconVu0FVECTOR LightColor[1];
  _iconVu0FVECTOR Ambient;
  unsigned char TitleName[1];
  unsigned char FnameView[1];
  unsigned char FnameCopy[1];
  unsigned char FnameDel[1];
...
typedef struct {
  ...
  short int prot;
  sceVu0FVECTOR ppos;
  sceVu0FVECTOR epos[1];
  sceVu0FVECTOR rgpos[1];
} STAGE_DAT;
...
enum { CS_NORMAL = 0, CS_SPECIAL1 = 1, CS_SPECIAL2 = 2, CS_SPECIAL3 = 3 };
...
enum { CD_LOADINIT = 0, CD_FILELOAD = 1, CD_FILEEND = 2 };
...
enum { BANM_NORM = 0, BANM_LOAD = 1, BANM_AFTLOAD = 2, BANM_TEXDISP = 3 };
...
typedef struct {
  sceVu0FVECTOR fpos[1];
  sceVu0IVECTOR ipos[1];
  byte clip[1];
  byte alp[1];
  U32DATA ts[1];
  U32DATA tt[1];
...
enum { CMOVE_KAGE = 0, CMOVE_KARASU = 1, CMOVE_KAIDAN = 2, CMOVE_NINGYO = 3 };
...
typedef struct {
  FOD_EFF_DATA *fix[1];
  FOD_EF_FADE_MDL *fade_mdl[1];
  sceVu0FVECTOR fire_pos[1];
...
typedef struct {
  SCN_ANM_MDL man_mdl[1];
  SCN_ANM_MDL furn_mdl[1];
  SCN_ANM_MDL item_mdl[1];
  SCN_ANM_MDL door_mdl[1];
  int man_mdl_tex[1];
  FOD_CTRL fod_ctrl;
  SCENE_FOG fog;
  SgCAMERA tmp_cam;
...
typedef struct {
  short int cdl_flg[1][1];
  short int flame_alpha[1];
  short int flame_shape[1];
  short int stflame_alpha[1];
  short int stflame_shape[1];
  short int flame_time[1];
  short int stflame_time[1];
} EV_CDL_DAT;
...
typedef struct {
  ...
  float ey[1];
  u_int pht_cnt_bak;
  short int bon_mov_sx[1];
  short int bon_mov_sy[1];
  short int bon_mov_ex[1];
  short int bon_mov_ey[1];
  ...
  byte dmg_eff_alp;
  byte dmg_wait_tm;
  byte dmg_num_mov_tim;
  byte dmg_str_mov_tim;
  byte dmg_eff_tim;
  byte ene_dead_tmr;
  byte ene_dead_alp;
  byte rmp_alp;
  byte hit_hp_ovr_ene;
  byte hit_hp_ovr_ene_wait;
} NEW_INF;
...
enum { GAME_OVER_TO_ALBUM = 0, GAME_OVER_QUIT_GAME = 1 };
...
enum { BIG_ALBM_CMD_SEAL = 0, BIG_ALBM_CMD_DEL = 1, BIG_ALBM_CMD_NUM = 2 };
...
typedef struct {
  ...
  byte pow_bak[1];
  byte pow_flr[1][1];
  byte film_or_perform;
  byte bonus_or_special;
  byte pfm_max[1];
} CAM_DSP;
...
enum { GET_FILE_BOOK = 1, GET_FILE_PHOT = 2, GET_FILE_ITEM = 3 };
...
enum { DTL_FILE0 = 0, DTL_FILE1 = 1, DTL_FILE2 = 2, DTL_PHOT = 3, DTL_NUM = 4 };
...
struct RELATION_PRT {
  ...
  SPRT_DAT *ssd_p;
  short int st_index[1];
  short int start_no;
  short int end_no;
  short int attribute;
  int csr_y;
  short int wk_scl[1][1];
  short int wk_pos[1][1];
...
enum { RCSR_NORM = 0, RCSR_LOAD = 1, RCSR_AFTLOAD = 2, RCSR_TEXDISP = 3 };
...
enum { HIMURO_B1 = 1, HIMURO_1F = 2, HIMURO_2F = 4, HIMURO_3F = 8 };
